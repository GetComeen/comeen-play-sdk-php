...
export async function importComponent(componentName: string, url: string, timeout: number = 8000): Promise<ComponentOptions> {
  return new Promise(async (resolve, reject) => {
    const module: IModule = await importModule(componentName, url, timeout);
...
export async function importModule(componentName: string, url: string, timeout: number = 8000): Promise<IModule> {
    return new Promise((resolve, reject) => {
      ...
      let check = function (tries: number) {
        ...
        if ((componentName in window)) {
          ...
          console.log('RESOLVING COMPONENT MODULE');
          modules[componentName] = module;
          resolve(module);
